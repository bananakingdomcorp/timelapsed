...
class CardEditModal extends React.Component {
  ...
  saveEdit = () => {
    ...
    Api().put(`/card/${this.props.data.id}/`, {Data: {Description: this.state.description, Name: this.state.title, Position: pos, Topic: this.props.board[this.props.topic].Data.id} } )
    .then((res) => {
      if (res.status === 200) {
        //Call redux. 
